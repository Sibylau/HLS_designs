...
int main ()
{
	...
	FILE *fp;
	fp = fopen("/curr/jieliu15/HLS_design/QR/op`$ROW`x`$COL`.bin","rb");
	...
	// 2 : GR
	for(int i = ROWS - 1; i >= UNROLL_FACTOR; i--)
	{
		if(A1[i - UNROLL_FACTOR][0] == 0 && A1[i][0] == 0)
		{
			continue;
		}
		else
		{
		MATRIX_T mag = qrf_mag(A1[i - UNROLL_FACTOR][0], A1[i][0]);
		MATRIX_T c = A1[i - UNROLL_FACTOR][0] / mag;
		MATRIX_T s = A1[i][0] / mag;
		A1[i - UNROLL_FACTOR][0] = mag;
		A1[i][0] = 0;
		for(int j = 1; j < COLS; j++)
		{
			qrf_mm(c, s, A1[i - UNROLL_FACTOR][j], A1[i][j]);
		}
		for(int j = 0; j < ROWS; j++)
		{
			qrf_mm(c, s, Q1[j][i - UNROLL_FACTOR], Q1[j][i]);
		}
		}
	}
	for(int i = UNROLL_FACTOR - 1; i > 0; i--)
	{
		...
		{
		MATRIX_T mag = qrf_mag(A1[i - 1][0], A1[i][0]);
		MATRIX_T c = A1[i - 1][0] / mag;
		MATRIX_T s = A1[i][0] / mag;
		A1[i - 1][0] = mag;
		A1[i][0] = 0;
		for(int j = 1; j < COLS; j++)
		{
			qrf_mm(c, s, A1[i - 1][j], A1[i][j]);
		}
		for(int j = 0; j < ROWS; j++)
		{
			qrf_mm(c, s, Q1[j][i - 1], Q1[j][i]);
		}
		}
	}
	...
	for(int i = 1; i < COLS; i++)
	{
		for(int j = ROWS - 1; j > i; j--)
		{
			...
			{
				MATRIX_T mag = qrf_mag(A1[j][i], A1[j - UNROLL_FACTOR][i]);
				MATRIX_T c = A1[j - UNROLL_FACTOR][i] / mag;
				MATRIX_T s = A1[j][i] / mag;
				A1[j - UNROLL_FACTOR][i] = mag;
				A1[j][i] = 0;
				for(int k = i + 1; k < COLS; k++)
				{
					qrf_mm(c, s, A1[j - UNROLL_FACTOR][k], A1[j][k]);
				}
				for(int k = 0; k < ROWS; k++)
				{
					qrf_mm(c, s, Q1[k][j - UNROLL_FACTOR], Q1[k][j]);
				}
			}
		}
